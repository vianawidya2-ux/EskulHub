                ...
                <div class="bg-white p-8 rounded-[35px] shadow-sm border border-slate-100 relative overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div>
                            ...
                            <input type="text" value="{{ Auth::user()->name }}" class="w-full bg-slate-100 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-500" readonly>
                        </div>

                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest">Unit Eskul</label>
                            <select name="unit_eskul" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-orange-500" required>
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ date('d F Y') }}" class="w-full bg-slate-100 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-500" readonly>
                        </div>

                        <div class="md:col-span-3">
                            <label class="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest">Status Kehadiran</label>
                            <select name="status" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500" required>
                    ...
                    <button type="submit" class="w-full bg-slate-800 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-orange-600 transition-all duration-300">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-all">
                                <td class="px-8 py-4 font-bold text-slate-700 text-sm">
                                    {{ $data->user?->name ?? $data->nama_manual ?? 'Siswa' }}
                                </td>
                                ...
                                <td class="px-8 py-4 text-center">
                                    <form action="{{ route('absensi.destroy', $data->id) }}" method="POST" onsubmit="return confirm('Hapus data?')">
                                        @csrf @method('DELETE')
